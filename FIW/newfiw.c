...
/* may want to disable subject checking entirely - or simply change it
* and "other" case for import_pkt for FREQ (use some nonsense string like
* AGGBX% FILENAME.EXT   (for UUE shipping)
* AGGBX! FILENAME.EXT   (for MIME shipping)
* so we don't have to fuck around with other shit
* inbound UUE - just screw it.
* Go ahead and put CRC-32 on outbound FIW stuff, and check anything that
* has it on inbound.  Use the freakin' SEAT protocol.  Who cares.  =)
* Can't decide whether to rip out BINKLEY support or not.
* Screw splitting and all that shit - that's entirely too complicated right
* now and not documented for anything.
*/
...
/************************************************************************
* This function checks for packet type (if any).  May wish to change
* subject line checking to JUST checking for inbound FREQ via some kind
* of "code" bit
************************************************************************/
 ...
 ************************************************************************/
  ...
  fread(&s1,61,1,check);   /* reading to get subject*/
  l=ftell(check);
  fclose(check);
  strupr(s1);
  if ((strstr(s1,"FI")) && (strstr(s1,"->")))   /*Regular FIW*/
   return(TRUE);
  check=fsh_open(s,"rt");  /*getting ready to search the hard way*/
  fseek(check,l,SEEK_SET);
  } else {
 ...
 s1[0]='\0';
 for(a=0;(a < 300) && (b<2);a++)  /*up to 300 lines in */
  {
  if (fgets(s,255,check)==NULL)
   b=3; 				 /*duh - EOF*/
  strupr(s);
  if (strstr(s,"X-MAILER: "))  /*snagging the x-mailer tag, compare later*/
   {
   char *p;  /*declaration */
  ...
  if (strstr(s,"SUBJECT: "))  /*checking that subject line again*/
   {
   if ((strstr(s,"FI")) && (strstr(s,"->")))   /*Regular FIW*/
    {
    fclose(check);
    return(TRUE);
    }
   b++;    /*found subject, not obvious FIW */
   continue;
   }
  }
 fclose(check);
...
/*added for TDS*/
	...
	fputc(fgetc(f),write);
     ...
     if (export==OTHER)  /* will NOT send out this run! Findnext should catch */
      ...
      f=fsh_open(s,"wb");
      sprintf(mh.from,"FIW");sprintf(mh.to,"SYSOP");
      printf("%s - file attach created\n",s1);
      sprintf(mh.subject,"%s",s1);
...
giveup_timeslice();
mailer=ARCMAIL;  /* Default, of course */
if ((readini())==0)
 {
